...
if (!isset($_GET['ticket']) || empty($_GET['ticket'])) fail(400);
else {
  echo json_encode(Array('ticket' => $_GET['ticket'], 'service'));
  $data = file_get_contents('https://cas.utc.fr/cas/serviceValidate?service='.$_GET['service'].'&ticket='.$_GET['ticket']);
  if (empty($data)) fail(403);
  else {
    $parsed = new xmlToArrayParser($data);
    if (!isset($parsed->array['cas:serviceResponse']['cas:authenticationSuccess']['cas:user'])) fail(403);
    else {
      echo json_encode($parsed->array['cas:serviceResponse']['cas:authenticationSuccess']['cas:user']);
    }
  }
